...
interface ChatPanelProps {
  ...
  onBulkApprove: (msgId: string, attachment: { name: string; data: string; mimeType: string } | null) => void;
...
function BulkApprovalCard({ msg, onApprove, onDecline }: {
  ...
  onApprove: (attachment: { name: string; data: string; mimeType: string } | null) => void;
  onDecline: () => void;
}) {
        ...
        {msg.sent ? (
          ...
          <div>
            ...
            <div style={{ background: "rgba(255,255,255,0.06)", borderRadius: "4px", height: "6px", overflow: "hidden" }}>
              <div style={{
                background: "linear-gradient(90deg, #F07C2D, #F59E0B)",
                height: "6px",
                borderRadius: "4px",
                width: msg.progress > 0 ? `${msg.progress * 100}%` : "100%",
                transition: "width 0.3s ease",
                animation: msg.progress === 0 ? "progressPulse 1.5s ease-in-out infinite" : "none"
              }} />
            </div>
          </div>
        ) : cancelled ? (
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
            <button onClick={() => onApprove(attachment)} style={{ background: "#22C55E", border: "none", borderRadius: "8px", color: "#fff", fontSize: "13px", fontWeight: 500, padding: "10px", cursor: "pointer", fontFamily: "Inter, sans-serif" }}>
              Approve & Send All
            </button>
           <button
  ...
  <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.6)", zIndex: 100, display: "flex", alignItems: "center", justifyContent: "center" }}>
    <div style={{ background: "#0D1128", border: "1px solid rgba(255,255,255,0.1)", borderRadius: "16px", padding: "24px", width: "520px", maxHeight: "80vh", overflowY: "auto" }}>
          ...
          <button
            ...
            onClick={async () => {
              ...
              try {
                ...
                const res = await fetch("https://outreach-ai-621913909275.us-central1.run.app/api/approvals/bulk/refine", {
                  method: "POST",
      ...
      <div style={{ marginTop: "12px" }}>
        ...
        />
        <label htmlFor="attachment-upload" style={{ display: "block", textAlign: "center", background: "transparent", border: "1px solid rgba(255,255,255,0.12)", borderRadius: "8px", color: "rgba(255,255,255,0.6)", fontSize: "13px", padding: "10px", cursor: "pointer" }}>
          {attachment ? `📎 ${attachment.name}` : "Attach File"}
        </label>
        {attachment && (
          <button onClick={() => setAttachment(null)} style={{ background: "none", border: "none", color: "#EF4444", fontSize: "12px", cursor: "pointer", marginTop: "4px", fontFamily: "Inter, sans-serif" }}>
            Remove attachment
          </button>
        )}
      </div>
                ...
                <>
                  ...
                  <div style={{ display: "flex", gap: "8px" }}>
                    ...
                    {reviewIndex < msg.previewLeads.length - 1 ? (
                      ...
                      <button onClick={() => { setReviewMode(false); onApprove(attachment); }} style={{ flex: 1, background: "#22C55E", border: "none", borderRadius: "8px", color: "#fff", fontSize: "13px", fontWeight: 500, padding: "10px", cursor: "pointer", fontFamily: "Inter, sans-serif" }}>
                        Approve & Send All
                      </button>
                    )}
                  </div>
...
function LeadHistoryView({ msg }: { msg: Extract<MessageType, { kind: "lead-history" }> }) {
  ...
  useEffect(() => {
    setLoading(true);
    fetch(`https://outreach-ai-621913909275.us-central1.run.app/api/history/${msg.lead.id}?user_id=${userId}`)
      .then(r => r.json())
              ...
              {filtered.map(item => {
                ...
                return (
                  <div key={item.id} style={{ display: "flex", flexDirection: "column", alignItems: isOutbound ? "flex-end" : "flex-start", marginBottom: "12px" }}>
                    ...
                    <div style={{
                      maxWidth: "85%",
                      background: isOutbound ? "rgba(240,124,45,0.12)" : "#1E2230",
                      border: `1px solid ${isOutbound ? "rgba(240,124,45,0.25)" : "rgba(255,255,255,0.06)"}`,
                      borderRadius: isOutbound ? "14px 14px 4px 14px" : "4px 14px 14px 14px",
                      padding: "10px 14px",
                    }}>
                      {item.subject && (
                        <p style={{ color: isOutbound ? "#F07C2D" : "#F0F1F5", fontSize: "12px", fontWeight: 600, marginBottom: "6px" }}>{item.subject}</p>
                      )}
                      {item.body && (
                        <p style={{ color: "rgba(255,255,255,0.7)", fontSize: "12px", lineHeight: 1.65, whiteSpace: "pre-wrap" }}>{item.body}</p>
                      )}
                      {item.reply_text && (
                        <p style={{ color: "rgba(255,255,255,0.7)", fontSize: "12px", lineHeight: 1.65, whiteSpace: "pre-wrap" }}>{item.reply_text}</p>
                      )}
                      {item.reply_classification && (
                        <span style={{
                          fontSize: "10px", fontWeight: 700, padding: "2px 7px", borderRadius: "4px", marginTop: "6px", display: "inline-block",
                          background: item.reply_classification === "POSITIVE" ? "rgba(34,197,94,0.12)" : item.reply_classification === "NEUTRAL" ? "rgba(245,158,11,0.12)" : item.reply_classification === "MEETING_DETAILS" ? "rgba(240,124,45,0.12)" : "rgba(239,68,68,0.12)",
                          color: item.reply_classification === "POSITIVE" ? "#22C55E" : item.reply_classification === "NEUTRAL" ? "#F59E0B" : item.reply_classification === "MEETING_DETAILS" ? "#F07C2D" : "#EF4444",
                      ...
                      {item.event_link && (
                        <a href={item.event_link} target="_blank" rel="noopener noreferrer" style={{ color: "#F07C2D", fontSize: "12px", marginTop: "6px", display: "block" }}>Join Meeting →</a>
                      )}
...
function AnalyticsCard({ msg }: { msg: Extract<MessageType, { kind: "analytics" }> }) {
  ...
  useEffect(() => {
    setLoading(true);
    fetch(`https://outreach-ai-621913909275.us-central1.run.app/api/analytics/${userId}?period=${filter}`)
      .then(r => r.json())
        ...
        {isEmpty ? (
          ...
          <>
            {messages.map(msg => {
              ...
              if (msg.kind === "bulk-approval") return <BulkApprovalCard key={msg.id} msg={msg} onApprove={(att) => onBulkApprove(msg.id, att)} onDecline={() => {}} />;
