...
export default function LoginPage({ onLogin }: LoginPageProps) {
  ...
  useEffect(() => {
    ...
    // ← ADD: check if already logged in via stored user_id
    const storedUserId = localStorage.getItem("user_id");
    if (storedUserId) {
      checkExistingAuth(storedUserId);
    }
  }, [onLogin]);

  const checkExistingAuth = async (email: string) => {
    try {
      const res = await fetch(`https://localhost:8080/api/auth/check/${email}`);
  ...
  const handleGoogleLogin = async () => {
    setLoading(true);
    try {
      const res = await fetch("https://outreach-ai-621913909275.us-central1.run.app/api/auth/google");
      const data = await res.json();
      if (data.auth_url) {
