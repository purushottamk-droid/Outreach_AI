...
export function TopNav({ sessionActive, sessionId, notificationCount, userId }: TopNavProps) {
  ...
  const fetchNotifications = () => {
    fetch(`https://outreach-ai-621913909275.us-central1.run.app/api/notifications/${uid}`)
      .then(r => r.json())
      .then((data: any[]) => {
  ...
  const handleApprove = async (notif: Notification) => {
    ...
    try {
      await fetch(`https://outreach-ai-621913909275.us-central1.run.app/api/approvals/${notif.id}/approve`, {
        method: "POST",
  ...
  const handleDecline = async (notif: Notification) => {
    try {
      await fetch(`https://outreach-ai-621913909275.us-central1.run.app/api/approvals/${notif.id}/skip`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ approval_id: notif.id, user_id: uid })
      });
  ...
  const handleRefine = async (notif: Notification) => {
    ...
    setRefining(true);
    try {
      const res = await fetch(`https://outreach-ai-621913909275.us-central1.run.app/api/approvals/${notif.id}/refine`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
  ...
  const getInitials = (id: string) => {
    if (!id) return "U";
    const name = id.split("@")[0];        // "purushottam.k"
    const parts = name.split(/[._-]/);    // ["purushottam", "k"]
  ...
  return (
    <>
        ...
        <div className="flex items-center gap-3">
          ...
          {/* Dashboard Button */}
            <button
              ...
              onMouseEnter={e => {
                (e.currentTarget as HTMLElement).style.background = "rgba(240,124,45,0.2)";
                (e.currentTarget as HTMLElement).style.borderColor = "rgba(240,124,45,0.5)";
              }}
              onMouseLeave={e => {
                (e.currentTarget as HTMLElement).style.background = "rgba(240,124,45,0.1)";
                (e.currentTarget as HTMLElement).style.borderColor = "rgba(240,124,45,0.25)";
              }}
            >
              📊 Dashboard
            </button>
      ...
      <div style={{
        ...
        <p style={{ color: "rgba(255,255,255,0.4)", fontSize: "11px", padding: "4px 8px", margin: 0 }}>
          {userId || uid}
        </p>
        <div style={{ height: "1px", background: "rgba(255,255,255,0.06)", margin: "6px 0" }} />
        {ADMIN_EMAILS.includes(uid) && (
          <button
            onClick={() => window.location.href = "/admin"}
            style={{
              width: "100%", background: "transparent", border: "none",
              color: "#F07C2D", fontSize: "13px", fontWeight: 500,
              padding: "6px 8px", cursor: "pointer", textAlign: "left",
              borderRadius: "6px", fontFamily: "Inter, sans-serif",
            }}
            onMouseEnter={e => (e.currentTarget as HTMLElement).style.background = "rgba(240,124,45,0.1)"}
