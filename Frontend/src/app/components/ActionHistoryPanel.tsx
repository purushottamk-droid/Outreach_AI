...
function CampaignsTab() {
  ...
  useEffect(() => {
    ...
    const loadCampaign = (isFirstLoad = false) => {
      fetch(`https://outreach-ai-621913909275.us-central1.run.app/api/campaigns/${uid}`)
        .then(r => r.json())
        .then((data: any[]) => {
          ...
          if (active) {
            ...
            if (isFirstLoad) {
              fetch(`https://outreach-ai-621913909275.us-central1.run.app/api/followup/config/${active.campaign_id}`)
                .then(r => r.json())
                .then(cfg => {
  ...
  const handleAskLlm = async () => {
    ...
    try {
      const uid = localStorage.getItem("user_id") || "user-001";
      const profileRes = await fetch(`https://outreach-ai-621913909275.us-central1.run.app/api/profile/${uid}`);
      const profile = await profileRes.json().catch(() => ({}));
      const industry = profile?.target_industry || "SaaS";

      const res = await fetch("https://outreach-ai-621913909275.us-central1.run.app/api/followup/recommend", {
          method: "POST",
  ...
  const handleSave = async () => {
    const uid = localStorage.getItem("user_id") || "user-001";
    try {
      await fetch("https://outreach-ai-621913909275.us-central1.run.app/api/followup/config/save", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
            user_id: uid,
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
      ...
      {allCampaigns.length > 1 && (() => {
          const totals = allCampaigns.reduce((acc, c) => ({
            leads: acc.leads + (c.lead_count || 0),
            contacted: acc.contacted + (c.emails_sent || 0),
            replied: acc.replied + (c.replies_received || 0),
            meetings: acc.meetings + (c.meetings_booked || 0)
          }), { leads: 0, contacted: 0, replied: 0, meetings: 0 });
          return (
            <div style={{ background: "rgba(240,124,45,0.07)", border: "1px solid rgba(240,124,45,0.2)", borderRadius: "12px", padding: "14px", marginBottom: "8px" }}>
              <p style={{ color: "#F07C2D", fontSize: "12px", fontWeight: 600, marginBottom: "8px", textTransform: "uppercase", letterSpacing: "0.06em" }}>All Campaigns</p>
              <div style={{ display: "flex", gap: "12px", flexWrap: "wrap" }}>
                {[
                  { label: "Leads", value: totals.leads, color: "rgba(255,255,255,0.6)" },
                  { label: "Contacted", value: totals.contacted, color: "#F07C2D" },
                  { label: "Replied", value: totals.replied, color: "#22C55E" },
                  { label: "Meetings", value: totals.meetings, color: "#34D399" },
                ].map(stat => (
      ...
      <div style={{ background: "#0D1128", border: "1px solid rgba(255,255,255,0.06)", borderRadius: "12px", padding: "10px" }}>
      ...
      <div style={{ background: "#0D1128", border: "1px solid rgba(255,255,255,0.06)", borderRadius: "12px", padding: "10px" }}>
        ...
        {/* Horizontal Timeline */}
        <div style={{ position: "relative", padding: "4px 8px 2px" }}>
          {/* Connecting lines */}
...
export function ActionHistoryPanel({
  ...
  onApprove, onDecline, onQuickAction, history,leads = [],collapsed,
  onCollapsedChange,
  ...
  return (
    <aside style={{ width: collapsed ? "40px" : "320px", minWidth: collapsed ? "40px" : "320px", background: "#0D0F12", borderLeft: "1px solid rgba(255,255,255,0.06)", display: "flex", flexDirection: "column", overflow: "hidden", fontFamily: "Inter, sans-serif", transition: "width 0.3s ease", position: "relative" }}>
      <button
        onClick={() => onCollapsedChange(!collapsed)}
        title="Campaign Hub"
        style={{ position: "absolute", top: "20px", left: collapsed ? "8px" : "auto", right: collapsed ? "auto" : "12px", background: "#1E2230", border: "1px solid rgba(255,255,255,0.1)", borderRadius: "6px", padding: "6px", cursor: "pointer", zIndex: 10 }}
      >
        <ChevronRight size={14} color="#F07C2D" style={{ transform: collapsed ? "rotate(180deg)" : "rotate(0deg)", transition: "transform 0.3s" }} />
      </button>
