...
const STAGES_ANALYTICS = [
  "Coordinator routing...", "Analytics Agent activated...",
  "Fetching data from BigQuery...", "Building report...",
];
...
async function classifyIntent(text: string): Promise<string> {
  try {
    const res = await fetch("https://outreach-ai-621913909275.us-central1.run.app/api/classify-intent", {
      method: "POST",
...
export default function App() {
  ...
  useEffect(() => {
      if (!userId) return;
      fetch(`https://outreach-ai-621913909275.us-central1.run.app/api/profile/${userId}`)
          .then(r => r.json())
          .then(data => {
              if (data && data.user_id) {
                  setSavedProfile({
                      ...
                      whatYouSell: data.product_service || "",
                  });
              }
          })
          .catch(() => {});
  ...
  useEffect(() => {
    if (!userId) return;
    fetch(`https://outreach-ai-621913909275.us-central1.run.app/api/history/user/${userId}`)
      .then(r => r.json())
      .then((data: any[]) => {
  ...
  useEffect(() => {
    if (!userId) return;
    fetch(`https://outreach-ai-621913909275.us-central1.run.app/api/approvals/${userId}`)
      .then(r => r.json())
  ...
  useEffect(() => {
    const handleDashboard = () => {
      handleSend("SHOW ME ANALYTICS");
    };
    window.addEventListener("dashboard-open", handleDashboard as EventListener);
    return () => window.removeEventListener("dashboard-open", handleDashboard as EventListener);
  }, []); 

  useEffect(() => {
    if (!userId) return;
    const ws = new WebSocket(`wss://outreach-ai-621913909275.us-central1.run.app/ws/${userId}`);
  ...
  const handleSend = async (text: string) => {
    ...
    const stages = intent === "analytics" ? STAGES_ANALYTICS
    : intent === "find-leads" ? STAGES_FIND
    : intent === "research" ? STAGES_RESEARCH
    : STAGES_MESSAGING;
  const duration = intent === "analytics" ? 3000
    : intent === "find-leads" ? 4000
    : intent === "research" ? 3800
    : 2600;
    ...
    // ── Analytics intent ──
     // Dashboard popup — broad analytics view
        if (intent === "analytics") {
      await runTyping(STAGES_ANALYTICS, 3000);
      try {
        ...
        const responseText = typeof response === "string"
          ? response
          : response?.agent_response || response?.raw_response || JSON.stringify(response);

        setIsTyping(false);

        // If analytics agent returns ANALYTICS_CARD → show dashboard widget
        if (responseText.includes("ANALYTICS_CARD")) {
          addMessage({ kind: "analytics", id: generateId(), timestamp: now() });
        } else {
          // contacted leads list, replied leads, campaign stats etc → show as text
          addMessage({
            kind: "agent",
            id: generateId(),
            agent: "coordinator",
        ...
        addMessage({
          ...
          text: "Could not fetch analytics. Please try again.",
          timestamp: now(),
        });
      }
      return;
    ...
    if (
      ...
      try {
        const res = await fetch(`https://outreach-ai-621913909275.us-central1.run.app/api/leads/history/${userId}`);
        const data = await res.json();
      ...
      try {
        ...
        if (typeof response === "string") {
          try {
            ...
            if (match) {
              try { parsedResponse = JSON.parse(match[0]); } catch {}
            }
          }
        }
...
const result = await api.runAgent({
  ...
  const handleGenerateBulkEmails = async (leads: LeadItem[]) => {
    ...
    try {
      const previewResult = await fetch("https://outreach-ai-621913909275.us-central1.run.app/api/preview-emails", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          user_id: userId,
  ...
  const handleBulkApprove = async (msgId: string, attachment: { name: string; data: string; mimeType: string } | null = null) => {
    setMessages(prev => prev.map(m =>
    ...
    try {
      let campaignId = null;
      try {
        const campaignResult = await fetch("https://outreach-ai-621913909275.us-central1.run.app/api/campaigns/create", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            user_id: userId,
  ...
  if (!isLoggedIn) {
   ...
   return (
    <div style={{ width: "100vw", height: "100vh", background: "#080B18", display: "flex", flexDirection: "column", overflow: "hidden", fontFamily: "Inter, sans-serif" }}>
      ...
      {historyModalLead && (
        <LeadHistoryModal lead={historyModalLead} onClose={() => setHistoryModalLead(null)} />
      )}

      <TopNav sessionActive={sessionActive} sessionId={sessionId} notificationCount={notificationCount} userId={userId} />
